...
public enum Paint_BrushMode{PAINT};
public class NavMeshPainter : MonoBehaviour {
	...
	public LayerMask navMeshMask = 1 << 11;

	Paint_BrushMode mode; //Our painter mode (Paint brushes or decals)
	...
	void DoAction(Color brushColor/*, Transform target*/){	
		if (saving)
			return;
		Vector3 uvWorldPosition=Vector3.zero;		
		if(HitTestUVPosition(ref uvWorldPosition/*, target*/)){
			GameObject brushObj;
	...
	void UpdateBrushCursor(){
		Vector3 uvWorldPosition=Vector3.zero;
		if (HitTestUVPosition (ref uvWorldPosition/*, brushCursor.transform*/) && !saving) {
	...
	bool HitTestUVPosition(ref Vector3 uvWorldPosition/*, Transform target*/){
		RaycastHit hit;
		//Vector3 cursorPos = new Vector3 (Input.mousePosition.x, Input.mousePosition.y, 0.0f);
		//Ray cursorRay=sceneCamera.ScreenPointToRay (cursorPos);
		if (/*Physics.Raycast(target.position,Vector3.down, out hit,200,navMeshMask)*/true){
	...
	// public void SetBrushMode(Paint_BrushMode brushMode){ //Sets if we are painting or placing decals
	// 	mode = brushMode;
